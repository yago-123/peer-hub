...
func (c *Client) Discover(ctx context.Context, peerID string) (*types.PeerResponse, *net.UDPAddr, error) {
	...
	udpAddr, err := net.ResolveUDPAddr(common.UDPProtocol, peerResp.Endpoint)
	if err != nil {
...
func (c *Client) WaitForPeer(ctx context.Context, peerID string) (*types.PeerResponse, *net.UDPAddr, error) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err == nil && res != nil && addr != nil {
				udpAddr, errUDP := net.ResolveUDPAddr(common.UDPProtocol, res.Endpoint)
				if errUDP != nil {
					return nil, nil, fmt.Errorf("invalid endpoint in response: %w", err)
				}
